...
export class Path {
  ...
  animate() {
    ...
    if (this.input.lastKey() == 'KeyR'){
      let enemy = this.enemySystem.enemies[0];
      enemy.fsm.transition('ENEMY_SPOTTED', { player: new Target(Math.floor(Math.random() * (this.environmentSystem.width)) - 20, 0, Math.floor(Math.random() * (this.environmentSystem.depth)) - 20) });
    }
